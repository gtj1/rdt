...
ImageGray = np.ndarray[tuple[int, int], np.dtype[np.uint8]]
ImageColor = np.ndarray[tuple[int, int, Literal[3]], np.dtype[np.uint8]]

class CameraFrame(TypedDict):
    rgbd_depth: ImageGray
    rgbd_image: ImageColor
    usb_image: ImageColor
...
CommandQueue = Queue[RobotCommand]
RecordQueue = Queue[RobotRecord]
...
class RobotController(Thread):
    ...
    undefined_pose: PoseEuler = (
        (np.nan, np.nan, np.nan), (np.nan, np.nan, np.nan)
    )
    undefined_joint_position: JointPosition = (
        np.nan, np.nan, np.nan, np.nan, np.nan, np.nan
    )
...
class CameraCollector:
    ...
    def __init__(self):
        ...
        self.usb_camera = cv2.VideoCapture(6)
    ...
    def shot(self) -> CameraFrame:
        ...
        for _ in range(try_times):
            success, frames = self.rgbd_camera.try_wait_for_frames()
            if success:
                break
            else:
                print("retrying...")
        ...
        if not frames:
            raise RuntimeError('Failed to get frames from RGBD camera')
        
        print("received frames from RGBD camera")
