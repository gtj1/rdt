...
import sys
sys.path.append("/home/star/Desktop/RoboticsDiffusionTransformer")

import random
from typing import Any, Literal
from data.core import RobotRecord, ImageColor
...
class HDF5VLADataset:
    ...
    def __init__(self):
        ...
        DATA_DIR = "data/datasets/my_cool_dataset"
        # DATA_DIR = "out/"
        self.DATASET_NAME = "my_cool_dataset"
        self.min_steps = 64
        ...
        for foldername in os.listdir(DATA_DIR):
            ...
            ret, indices = self.read_episodes(folder_path)
            if ret:
        ...
        for _, indices in zip(self.record_paths, self.indices):
    ...
    def init_states_and_actions(path: str, indices: range):
        ...
        for index in indices:
            ...
            ortho6d: RotationRepresentation = rotation_matrix[:, :2].transpose().flatten()
            ...
            joint = arm_state['joint_position']
            xyz, rpy = arm_state['end_effector_pose']
            rotation_matrix = R.from_euler('xyz', list(rpy), degrees=False).as_matrix()
    ...
    def read_episodes(
        cls, 
        path: str, 
        pattern: str | re.Pattern[str] = r'^episode_(\d+)$'
    ) -> tuple[Literal[False], str] | tuple[Literal[True], range]:
        """
        ...
        # It's strange that listdir returns episode: bytes for path: str.
        # A temporary fix for this issue
        if isinstance(episodes[0], bytes):
            episodes = [episode.decode() for episode in episodes]
        # for filename in os.listdir(path):
        #     if filename.endswith('.npy'):
        #         npy_files.append(filename)
        if isinstance(pattern, str):
            pattern = re.compile(pattern)
        matches = [pattern.match(filename) for filename in episodes]
        ...
        for filename_match in matches:
            ...
            new_index_str = f"episode_{int(index_str)}"
            episode_name = os.path.join(path, filename_match.group(0))
            if index_str != new_index_str:
                new_name = os.path.join(path, new_index_str)
                os.rename(episode_name, new_name)
                episode_name = new_name
            if not os.path.exists(os.path.join(episode_name, "states.npy")):
                npy_matches = [re.match(r"(\d+).npy", filename) for filename in os.listdir(episode_name)]
                npy_indices = [int(filename_match.group(1)) for filename_match in npy_matches if filename_match]
                if not npy_indices:
                    return (False, f"Error when reading {episode_name}")
                expected_npy_range = range(min(npy_indices), max(npy_indices) + 1)
                if len(set(npy_indices)) != len(expected_npy_range):
                    return (False, f"Error when reading {episode_name}")
                cls.init_states_and_actions(episode_name, expected_npy_range)
        return (True, expected_range)
    
    def read_episode_indices(self, episode_name: str):
        npy_matches = [re.match(r"(\d+).npy", filename) for filename in os.listdir(episode_name)]
        npy_indices = [int(filename_match.group(1)) for filename_match in npy_matches if filename_match]
        if not npy_indices:
            return (False, f"Error when reading {episode_name}")
        ...
        if len(set(npy_indices)) != len(expected_npy_range):
            return (False, f"Error when reading {episode_name}")
        return True, range(min(npy_indices), max(npy_indices) + 1)
    ...
    def get_item(self, index: int | None=None, state_only: bool=False):
        ...
        while True:
            ...
            if valid:
                ...
                index = random.randint(0, len(self.record_paths) - 1)
    
    def parse_npy_file(self, episode_dir: str) -> tuple[bool, dict[str, Any] | None]:
        ...
        """
        ret, episode_indices = self.read_episodes(episode_dir)
        if not ret:
            return False, None
        episode_path = os.path.join(episode_dir, "episode_" + str(random.choice(episode_indices)))
        ret, record_indices = self.read_episode_indices(episode_path)
        if not ret:
            return False, None
        else:
            assert isinstance(record_indices, range)
        states: StateVectorChunk = np.load(os.path.join(episode_path, "states.npy"))
        actions: StateVectorChunk = np.load(os.path.join(episode_path, "actions.npy"))
        ...
        step_id = record_indices.start + random.randint(first_idx - 1, num_steps - 1)

        # Load the instruction
        with open(os.path.join(episode_dir, 'instruction.json'), 'r') as f_instr:
        ...
        if target_actions.shape[0] < self.CHUNK_SIZE:
            target_actions = np.concatenate([
                target_actions,
                np.tile(target_actions[-1], (self.CHUNK_SIZE - target_actions.shape[0], 1))
            ], axis=0)
        
        # Fill the state/action into the unified vector
        def fill_in_state(values: StateVectorChunk) -> UnifiedVectorChunk:
        ...
        def parse_image(keys: list[str]) -> dict[str, ImageType]:
            ...
            for i in range(max(step_id - self.IMG_HISORY_SIZE + 1, 0), step_id + 1):
                npy_filename = os.path.join(episode_path, f"{i}.npy")
        ...
        # print(f"\033[31mshape: expected a 3d tensor, got shape {state_indicator.shape}\033[0m")
        return True, {
            "meta": meta,
            "state": current_state,
            "state_std": state_std,
    ...
    def parse_npy_file_state_only(self, episode_dir: str):
        ...
        def fill_in_state(values: StateVectorChunk) -> UnifiedVectorChunk:
            # Target indices corresponding to your state space
            # concat [joint, xyz, ortho6d, gripper_position]
            UNI_STATE_INDICES = [
                STATE_VEC_IDX_MAPPING[f"arm_joint_{i}_pos"] for i in range(6)
            ] + [
                STATE_VEC_IDX_MAPPING[f"eef_pos_{ax}"] for ax in "xyz"
